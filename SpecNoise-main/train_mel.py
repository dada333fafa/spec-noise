...
def load_data(input_path, batch_size, seed):
    ...
    data_loader = BirdSoundLoader(input_path=input_path, test=0.2, random_seed=seed, feature_type='mel')
...
def save_model(model, checkpoint_dir, final_epoch_info, args):
    ...
    model_path = os.path.join(checkpoint_dir, f'output_mel_{aug_str}_{timestamp}.pth')
    torch.save(model.state_dict(), model_path)
    history_path = os.path.join(checkpoint_dir, f'history_training_mel_{aug_str}_{timestamp}.json')
