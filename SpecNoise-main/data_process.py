...
def process_librosa(input_path, output_path, offset, duration, n_mels, hop_length, arr_length):
    ...
    y, sr = librosa.load(input_path, offset=offset, duration=duration, sr=16000)
    padding_length = 16000 - len(y)
    pad_before = padding_length // 2
    pad_after = padding_length - pad_before
    y = np.pad(y, (pad_before, pad_after), 'wrap')
...
ap.add_argument("-i", "--input", required=True, help=r"D:\spec-noise\SpecNoise-main\dataset")
ap.add_argument("-o", "--output", required=False, help=r"D:\spec-noise\SpecNoise-main\images_dataset",
                default='./img_dataset')
ap.add_argument("-s", "--split", required=False, help="split time interval in seconds. Default 1 sec", default=1.0,
                type=float)
ap.add_argument("-ov", "--overlap", required=False,
                help="overlap percentage in case of multiple splits. Must be between 0 and 0.5. Default 0.2",
                default=0.2, type=check_percentage)
